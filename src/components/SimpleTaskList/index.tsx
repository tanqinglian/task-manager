...
interface SimpleTaskListProps {
  ...
  onConvertToFlashNote: (task: Task) => void;
}

export function SimpleTaskList({ onEditTask, onConvertToFlashNote }: SimpleTaskListProps) {
                ...
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        onConvertToFlashNote(task);
                      }}
                      className="p-2.5 rounded-xl hover:bg-purple-50 text-purple-500 transition-colors"
                      title="转为灵光一闪"
                    >
                      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
